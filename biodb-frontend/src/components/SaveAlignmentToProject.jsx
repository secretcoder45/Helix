...
import { useProjects, useCreateProject, useSaveAlignment } from '../lib/api'

/**
 * Save an alignment result into a project — the counterpart to
 * SaveToProject/SaveSelectionToProject for analyses rather than records.
 * Projects hold both, so a project is the full account of a piece of work.
 */
export function SaveAlignmentToProject({ alignment }) {
  ...
  const saveAlignment = useSaveAlignment()

  const save = async (projectId, projectName) => {
    await saveAlignment.mutateAsync({ projectId, alignment })
  ...
  const busy = saveAlignment.isPending || createProject.isPending
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        disabled={busy}
        className={`inline-flex h-9 items-center gap-1.5 rounded-lg border px-3.5 text-[13px] font-medium transition-colors disabled:opacity-50 ${
          savedTo
      ...
      >
        {savedTo ? (
          <>
            <Check size={14} /> Saved to {savedTo}
          </>
        ) : busy ? (
          <>
          ...
          <>
            <BookmarkPlus size={14} /> Save alignment
          </>
        )}
      </button>
          ...
          <>
            ...
            >
              ...
              {projects.length > 0 ? (
                ...
                <p className="px-3 py-3 text-[12px] text-ink-3">No projects yet — name one below.</p>
              )}
